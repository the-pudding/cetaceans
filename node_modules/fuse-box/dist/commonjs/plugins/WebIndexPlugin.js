...
class WebIndexPluginClass {
    ...
    producerEnd(producer) {
        ...
        producer.bundles.forEach((bundle, name) => {
            ...
            if (pass) {
                bundlePaths.push(path.join(this.opts.path ? this.opts.path : "/", bundle.context.output.lastPrimaryOutput.filename));
            }
        });
...
$bundles
        ...
        let jsTags = bundlePaths.map(bundle => `<script type="text/javascript" src="${bundle}"></script>`).join("\n");
        let macro = {
